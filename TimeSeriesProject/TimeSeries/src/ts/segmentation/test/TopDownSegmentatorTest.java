...
public class TopDownSegmentatorTest {
	
	private TimeLine t;
	private TopDownSegmentator topDown;
	private ArrayList<Integer> tIndex;
	public TopDownSegmentatorTest() {
		topDown = new TopDownSegmentator();
	...
	public void makeTests(int size, double maxError) {
		...
		TimeLine segmented = topDown.segmentationAlgorithm(t, maxError);
		...
		for(int i = 0; i < segmented.getSize() -1; i++) {
			...
			double error = topDown.calculateError(t, index1, index2);
			boolean b = false;
			if(error <= maxError) {
				b = true;
			}
